...
Table.prototype.refresh = function (callback) {
  ...
  // refresh the metadata to get accurate row and column counts
  this.metadata(function (err) {
    if (err) {
      self.emit('error', err);
    ...
    // query for new row anc column headers
...
Table.prototype.metadata = function (callback) {
  var self = this;
  debug('fetching worksheet metadata ..');
  this.worksheet.metadata(function (err, metadata) {
    if (err) {
      if (callback) callback(err);
      return;
    }

    extend(self.worksheet, metadata);
    debug('fetched worksheet metadata.');
...
Table.prototype.query = function (callback) {
  ...
  query.get(function (err, cells) {
    ...
    for (var c = 0; c < self.worksheet.columnCount; c += 1) {
      cell = cells[++index];
      if (cell && cell.value) self.columns[cell.value.toLowerCase()] = cell.column;
    }

    for (var r = 0; r < self.worksheet.rowCount; r += 1) {
      cell = cells[++index];
      if (cell && cell.value) self.rows[cell.value] = cell.row;
    }
...
Table.prototype.addColumn = function (key) {
  var lastColumn = max(this.columns) + 1;
...
/**
 * Clears flush and refresh intervals and
...
/**
 * Find the maximum value in a map.
 *
 * @param {Object} map
 * @return {Number}
 */
